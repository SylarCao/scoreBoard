...
- (void)determineCurrentRow
{
    ...
    if ([delegate respondsToSelector:@selector(pickerView:didSelectRow:)])
    {
...
- (void)makeSteps:(int)steps
{
    ...
    currentRow = MIN(currentRow + steps, rowsCount-2);// currentRow + steps;
...
- (UIToolbar *)toolbar:(NSString *)title {
    ...
    UIBarButtonItem *doneButton = [[UIBarButtonItem alloc] initWithTitle:@"确定" style:UIBarButtonItemStyleDone target:nil action:@selector(BtnOK)];
    ...
    UIBarButtonItem* cancelButton = [[UIBarButtonItem alloc] initWithTitle:@"取消" style:UIBarButtonItemStyleDone target:nil action:@selector(BtnCancel)];
...
- (void) BtnOK
{
    if (!isHidden) {
        [UIView animateWithDuration:0.3 animations:^(void) {
            self.frame = CGRectMake(0, self.frame.origin.y + self.frame.size.height, self.frame.size.width, self.frame.size.height);
            if ([delegate respondsToSelector:@selector(pickerView:PressOk:)])
            {
                [delegate pickerView:self PressOk:currentRow];
            }
        }];
        self.isHidden = YES;
    }
...
- (void) BtnCancel
{
    if (!isHidden) {
        [UIView animateWithDuration:0.3 animations:^(void) {
            self.frame = CGRectMake(0, self.frame.origin.y + self.frame.size.height, self.frame.size.width, self.frame.size.height);
            if ([delegate respondsToSelector:@selector(pickerView:PressCancel:)])
            {
                [delegate pickerView:self PressCancel:nil];
            }
        }];
        self.isHidden = YES;
    }
