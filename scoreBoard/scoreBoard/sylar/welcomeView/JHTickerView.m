...
{
    id rt = [self initWithFrame:frame Font:[UIFont fontWithName:@"Marker Felt" size:22.0]];
    return rt;
}
...
-(id)initWithCoder:(NSCoder *)aDecoder {
	if( (self = [super initWithCoder:aDecoder]) ) {
		// Initialization code
        tickerFont = [UIFont fontWithName:@"HelveticaNeue-CondensedBold" size:33];
		[self setupView];
	}
	return self;
}
...
//	tickerFont = [UIFont fontWithName:@"Marker Felt" size:22.0];
	...
	tickerLabel = [[RTLabel alloc] initWithFrame:CGRectMake(0, self.frame.size.height / 4, self.frame.size.width, self.frame.size.height)];
	[tickerLabel setBackgroundColor:[UIColor clearColor]];
    [tickerLabel setBackgroundColor:[UIColor lightGrayColor]];
...
//	CGSize textSize = [currentString sizeWithFont:tickerFont constrainedToSize:CGSizeMake(9999, self.frame.size.height) lineBreakMode:NSLineBreakByWordWrapping];
    CGRect textRect = [[JHTickerView removeHTML:currentString] boundingRectWithSize:CGSizeMake(99999, self.frame.size.height) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: tickerFont} context:nil];
